...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    """
    Stream graph progress using stream_mode="values" (full state after each node).
    The last yielded value IS the final state — no second invoke needed.
    Falls back to a plain invoke if streaming is unavailable.
    """
    last_value = None
    try:
        ...
        if last_value is not None:
            yield ("final", last_value)
            return
    except Exception as e:
        yield ("values", {"__stream_error__": str(e)})

    # Fallback: plain invoke (no streaming progress, but correct result)
...
if run_btn:
    ...
    for kind, payload in try_stream(app, inputs):
        if kind == "values":
            # stream_mode="values" yields full state dict keyed by state fields
            # Detect node completion by watching which keys changed vs current_state
            if isinstance(payload, dict) and "__stream_error__" in payload:
                status.write(f"Stream error: {payload['__stream_error__']} — falling back to invoke")
            else:
            ...
            new_node = None
            if current_state.get("mode") and not last_node:
                new_node = "router"
            elif current_state.get("evidence") and last_node == "router":
                new_node = "research"
            elif current_state.get("plan") and last_node in (None, "router", "research"):
                new_node = "orchestrator"
            elif current_state.get("sections") and last_node == "orchestrator":
                new_node = "worker_loop"
            elif current_state.get("final") and last_node == "worker_loop":
                new_node = "reducer"
            if new_node and new_node != last_node:
                status.write(f"Node: `{new_node}`")
                last_node = new_node
